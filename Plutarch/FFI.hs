...
unsafeForeignExport config t = DeserializedCode (fmap (const mempty) program) Nothing mempty
  where
...
unsafeForeignImport c = Term $ const $ pure $ TermResult (RCompiled $ UPLC._progTerm $ toNameless $ void $ getPlc c) []
  where
    toNameless ::
